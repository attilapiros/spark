...
private[deploy] class Worker(
  ...
  if (CLEANUP_NON_SHUFFLE_FILES_ENABLED && EXTERNAL_SHUFFLE_SERVICE_ENABLED) {
    logWarning("Both 'spark.storage.cleanupFilesAfterExecutorExit' and " +
      "'spark.shuffle.service.enabled' are switched on. But from SPARK-25888 the external " +
      "shuffle service able to serve disk persisted RDD blocks. So to profit from this feature " +
      "cleanup only triggered after the application is stopped.")
  }
